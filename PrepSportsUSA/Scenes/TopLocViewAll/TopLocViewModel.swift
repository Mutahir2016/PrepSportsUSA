...
class TopLocViewModel: BaseViewModel {
    
    var viewAllUseCase: ViewAllUseCase?
    let errorSubject = PublishSubject<Error>()
    let isLoadingRelay = BehaviorRelay<Bool>(value: false)

    private(set) var topLocationArr = [GeographyData]()
    ...
    func viewDidLoad() {
        if let fromDate = getFromDate(), let toDate = getToDate() {
            ...
            fetchLocations(fromDate: fromDate, toDate: toDate, isComingFromNetwork: self.isComingFromNetwork)
        } else {
            ...
            fetchLocations(fromDate: fromDate.formatted(template: "yyyy-MM-dd"), toDate: toDate.formatted(template: "yyyy-MM-dd"), isComingFromNetwork: self.isComingFromNetwork)
        }
    }
    ...
    func fetchLocations(fromDate: String, toDate: String, isComingFromNetwork: Bool = false) {
        if hasLoaded {
            return
        }
        ...
        viewAllUseCase?
            .getStoryGeography(storyId: storyId.value,
            ...
            .subscribe(onNext: { [weak self] topLocModel in
                ...
                if let topLocObj = topLocModel {
                    ...
                    if topLocObj.data .isEmpty {
                        ...
                        self.topLocationArr.append(contentsOf: topLocObj.data)
                    }
