...
class OutboundViewController: BaseViewController {

    @IBOutlet weak var tableView: UITableView!
    @IBOutlet weak var dateRangeLbl: UILabel!
    var viewModel: OutboundViewModel!
    var router: OutboundRouter!
    ...
    override func callingInsideViewDidLoad() {
        ...
        self.title = "Outbound Clicks"
    }
    
    override func setUp() {
        router = OutboundRouter(self)
    }
    ...
    func showDateRangeSelector() {
        ...
        fastisController.doneHandler = { date in
            ...
            if let selectedRange = date {
                ...
                self.noDataLabel.isHidden = true
                self.viewModel.fetchOutboundClicks(fromDate: fromDateValue, toDate: toDateValue)
...
extension OutboundViewController: StoriesHomeViewModelDelegate {
    func reloadTableData() {
        DispatchQueue.main.async {
            if self.viewModel.topOutlinkArr.isEmpty {
...
extension OutboundViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModel.topOutlinkArr.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let topLoc = viewModel.topOutlinkArr[indexPath.row]
        cell.separatorView.isHidden = (indexPath.row == viewModel.topOutlinkArr.count - 1)

        cell.configView(topLoc)
        cell.leadingConstraint.constant = 20
        cell.trailingConstraint.constant = 20
        // Trigger pagination
        if indexPath.row == viewModel.topOutlinkArr.count - 1 && viewModel.currentPage < viewModel.totalRecords - 1 {
            viewModel.currentPage += 1
            
            self.viewModel.fetchOutboundClicks(fromDate: viewModel.getFromDate() ?? "",
...
extension OutboundViewController: TabBarDelegate {
...
extension OutboundViewController {
