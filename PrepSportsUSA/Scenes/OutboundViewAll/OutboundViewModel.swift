...
class OutboundViewModel: BaseViewModel {
    
    var viewAllUseCase: ViewAllUseCase?
    let errorSubject = PublishSubject<Error>()
    let isLoadingRelay = BehaviorRelay<Bool>(value: false)

    private(set) var topOutlinkArr = [OutlinkData]()
    ...
    func viewDidLoad() {
        if let fromDate = getFromDate(), let toDate = getToDate() {
            ...
            fetchOutboundClicks(fromDate: fromDate, toDate: toDate)
        } else {
            ...
            fetchOutboundClicks(fromDate: fromDate.formatted(template: "yyyy-MM-dd"), toDate: toDate.formatted(template: "yyyy-MM-dd"))
        }
    }
    ...
    func fetchOutboundClicks(fromDate: String, toDate: String) {
        if hasLoaded {
            return
        }
        ...
        viewAllUseCase?
            .getStoryOutlink(storyId: storyId.value,
                                       pageNumber: currentPage,
                                       fromDate: fromDate,
                                       toDate: toDate,
                             isComingFromNetwork: self.isComingFromNetwork)
            ...
            .subscribe(onNext: { [weak self] topLocModel in
                ...
                if let topLocObj = topLocModel {
                    ...
                    if topLocObj.data .isEmpty {
                        ...
                        self.topOutlinkArr.append(contentsOf: topLocObj.data)
                    }
